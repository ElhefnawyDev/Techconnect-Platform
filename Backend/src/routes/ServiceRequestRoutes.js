...
router.post("/", async (req, res) => {
  ...
  if (!customerId || !title || !description || !category || !budgetMin || !budgetMax) {
...
router.post("/:requestId/proposals", upload.single("attachment"), async (req, res) => {
    ...
    try {
      ...
      return res.status(201).json({ message: "Proposal submitted", proposal: newProposal });
    } catch (err) {
...
router.delete("/dservice-request/:id", async (req, res) => {
  ...
  try {
    // Step 1: Delete all proposals related to this request
    await prisma.proposal.deleteMany({
      where: { requestId: id },
    });

    // Step 2: Now delete the service request
    const deletedRequest = await prisma.serviceRequest.delete({
      where: { id },
    });

    res.status(200).json({ message: "Service request and related proposals deleted", deletedRequest });
  } catch (error) {
